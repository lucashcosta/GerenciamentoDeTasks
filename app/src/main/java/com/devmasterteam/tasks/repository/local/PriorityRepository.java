...
public class PriorityRepository {
        ...
        try {
            ...
            if (cursor != null && cursor.getCount() > 0) {
                while (cursor.moveToNext()) {
                    ...
                    guestEntity.Id = (cursor.getInt(cursor.getColumnIndexOrThrow(DataBaseConstants.PRIORITY.COLUMNS.ID)));
                    guestEntity.Description = (cursor.getString(cursor.getColumnIndexOrThrow(DataBaseConstants.PRIORITY.COLUMNS.DESCRIPTION)));
    ...
    public void insert(List<PriorityEntity> list) {
        ...
        for (PriorityEntity priorityEntity : list) {
            stmt.bindLong(1, priorityEntity.Id);
            stmt.bindString(2, priorityEntity.Description);
